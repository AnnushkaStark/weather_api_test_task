...
async def get_weather_in_city(
    ...
    if weather is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Wrong city name")
...
async def read_search_history(
    ...
    found_cities = await crud_city.get_multi_by_user_id(db=db, user_id=current_user.id)
    return found_cities
